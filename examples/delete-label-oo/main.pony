    ...
    try
      ...
      GitHub(creds).get_repo(owner, repo)
        .flatten_next[DeletedOrError](
          RemoveLabel~apply(name))
        .next[None](
          PrintResult~apply(env.out, name))
    else
...
primitive RemoveLabel
  """
  Deletes a label from a repository result.
  """
  fun apply(
    label: String,
    r: RepositoryOrError)
    : Promise[DeletedOrError]
  =>
    match \exhaustive\ r
    | let repo: Repository =>
...
primitive PrintResult
  """
  Prints the result of a label deletion.
  """
  fun apply(
    out: OutStream,
    label: String,
