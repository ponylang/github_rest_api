...
actor Main
  new create(env: Env) =>
    try
      let cs =
        CommandSpec.leaf(
          ...
          [
            ...
            OptionSpec.string(
              "since",
              "Only issues updated at or after " +
      ...
      let per_page: (I64 | None) =
        if per_page_i > 0 then
          per_page_i
        else
          None
        end
      ...
      GitHub(creds).get_repo(owner, repo)
        .flatten_next[
          (PaginatedList[Issue] | RequestError)](
          RetrieveIssues~apply(
            labels,
            state,
            sort,
            direction,
...
primitive RetrieveIssues
  """
  Retrieves issues from a repository result.
  """
  fun apply(
    labels: String,
    state: String,
...
primitive PrintIssues
  """
  Prints a paginated list of issues to the given output stream.
  """
  fun apply(
    out: OutStream,
