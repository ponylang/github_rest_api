  ...
  new create(env: Env) =>
    try
      let cs =
        CommandSpec.leaf(
          ...
          [
            ...
            OptionSpec.i64(
              "issue",
              "Issue number to get comments for")
            OptionSpec.string(
              "token",
      ...
      GitHub(creds).get_repo(owner, repo)
        ...
        .flatten_next[IssueCommentsOrError](
          RetrieveComments~apply())
        .next[None](
          PrintIssueComments~apply(env.out))
    else
...
primitive RetrieveComments
  """
  Retrieves comments from an issue result.
  """
  fun apply(
    i: IssueOrError)
