...
class \nodoc\ _TestSearchNextPageFollowsLink is UnitTest
  fun name(): String =>
    "search-pagination/search/next-page-follows-link"
  ...
  fun ref apply(h: TestHelper) ? =>
    ...
    let p =
      Promise[(SearchResults[String] | req.RequestError)]
    p.next[None](
      {(result:
                ...
                | let sr2: SearchResults[String] =>
                  try
                    ...
                    h.assert_true(
                      ...
                      "page 2 next_page should " +
                        "be None")
                    h.complete(true)
                  else
                    h.fail(
                      "Failed to access page 2 items")
                    h.complete(false)
                  end
                | let e: req.RequestError =>
                  h.fail(
                    "Page 2 error: " + e.message)
                  h.complete(false)
                end
              })
          | None =>
            h.fail(
              "next_page() returned None on page 1")
            h.complete(false)
          end
        | let e: req.RequestError =>
    ...
    let receiver =
      SearchResultReceiver[String](creds, p, converter)
    let listener =
      _MockHTTPListener(
        h,
        port,
...
class \nodoc\ _TestSearchPrevPageFollowsLink is UnitTest
  fun name(): String =>
    "search-pagination/search/prev-page-follows-link"
                ...
                | let sr2: SearchResults[String] =>
                  try
                    ...
                    h.assert_true(
                      ...
                      "page 1 prev_page should " +
                        "be None")
                    h.complete(true)
                  else
                    h.fail(
                      "Failed to access page 1 items")
                    h.complete(false)
                  end
                | let e: req.RequestError =>
                  h.fail(
                    "Page 1 error: " + e.message)
                  h.complete(false)
                end
              })
          | None =>
            h.fail(
              "prev_page() returned None on page 2")
            h.complete(false)
          end
        | let e: req.RequestError =>
...
class \nodoc\ _TestListNextPageFollowsLink is UnitTest
  fun name(): String =>
    "search-pagination/list/next-page-follows-link"
  ...
  fun ref apply(h: TestHelper) ? =>
    ...
    let p =
      Promise[(PaginatedList[String] | req.RequestError)]
    p.next[None](
      {(result:
      ...
      =>
        ...
        | let pl: PaginatedList[String] =>
          try
            ...
            h.fail(
              "Failed to access page 1 results")
            h.complete(false)
            return
          end
...
class \nodoc\ _TestListPrevPageFollowsLink is UnitTest
  fun name(): String =>
    "search-pagination/list/prev-page-follows-link"
      ...
      =>
        ...
        | let pl: PaginatedList[String] =>
          try
            ...
            h.fail(
              "Failed to access page 2 results")
            h.complete(false)
            return
          end
