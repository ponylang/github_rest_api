    ...
    p.next[None](
      {(result: req.DeletedOrError)(h) =>
        ...
        | let e: req.RequestError =>
          h.fail(
            "Expected Deleted, got RequestError: " +
              e.message)
          h.complete(false)
        end
      })
  ...
  fun ref apply(h: TestHelper) =>
    ...
    p.next[None](
      {(result: req.DeletedOrError)(h) =>
        ...
        | let _: req.Deleted =>
          h.fail(
            "Expected RequestError, got Deleted")
          h.complete(false)
        | let e: req.RequestError =>
          h.assert_eq[U16](404, e.status)
          h.assert_eq[String](
            "not found", e.response_body)
...
class \nodoc\ _TestBoolResultReceiverSuccessTrue is UnitTest
  fun name(): String =>
    "result-receivers/bool/success-true"
  ...
  fun ref apply(h: TestHelper) =>
    ...
    p.next[None](
      {(result: req.BoolOrError)(h) =>
        ...
        | let e: req.RequestError =>
          h.fail(
            "Expected Bool, got RequestError: " +
              e.message)
          h.complete(false)
        end
      })
...
class \nodoc\ _TestBoolResultReceiverSuccessFalse is UnitTest
  fun name(): String =>
    "result-receivers/bool/success-false"
  ...
  fun ref apply(h: TestHelper) =>
    ...
    p.next[None](
      {(result: req.BoolOrError)(h) =>
        ...
        | let _: Bool =>
          h.fail(
            "Expected RequestError, got Bool")
          h.complete(false)
        | let e: req.RequestError =>
          h.assert_eq[U16](422, e.status)
          h.assert_eq[String](
            "unprocessable", e.response_body)
          h.assert_eq[String](
  ...
  fun ref apply(h: TestHelper) =>
    ...
    p.next[None](
      {(result: (String | req.RequestError))(h) =>
        ...
        | let e: req.RequestError =>
          h.fail(
            "Expected String, got RequestError: " +
              e.message)
          h.complete(false)
        end
      })
...
class \nodoc\ _TestResultReceiverConverterError is UnitTest
  fun name(): String =>
    "result-receivers/json/converter-error"
  ...
  fun ref apply(h: TestHelper) =>
    ...
    p.next[None](
      {(result: (String | req.RequestError))(h) =>
        ...
        | let _: String =>
          h.fail(
            "Expected RequestError, got String")
          h.complete(false)
        | let e: req.RequestError =>
          h.assert_true(
            e.message.contains(
              "Unable to convert json"),
            "Expected 'Unable to convert json'" +
              ", got: " + e.message)
          h.complete(true)
        end
      })
...
class \nodoc\ _TestPaginatedResultReceiverSuccess is UnitTest
  fun name(): String =>
    "result-receivers/paginated/success"

  fun ref apply(h: TestHelper) =>
    h.long_test(2_000_000_000)
    let p =
      Promise[
        (PaginatedList[String] | req.RequestError)]
    p.next[None](
      {(result:
        (PaginatedList[String] | req.RequestError))
        (h)
...
class \nodoc\ _TestPaginatedResultReceiverFailure is UnitTest
  fun name(): String =>
    "result-receivers/paginated/failure"
...
class \nodoc\ _TestSearchResultReceiverSuccess is UnitTest
  fun name(): String =>
    "result-receivers/search/success"

  fun ref apply(h: TestHelper) =>
    h.long_test(2_000_000_000)
    let p =
      Promise[
        (SearchResults[String] | req.RequestError)]
    p.next[None](
      {(result:
        (SearchResults[String] | req.RequestError))
        (h)
...
class \nodoc\ _TestSearchResultReceiverFailure is UnitTest
  fun name(): String =>
    "result-receivers/search/failure"
