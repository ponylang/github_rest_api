...
class val Repository
  ...
  fun create_label(
    ...
    label_description: (String | None) = None)
  ...
  fun delete_label(label_name: String)
  ...
  =>
    """
    Lists issues in this repository, optionally filtered by labels
    and state. Results can be sorted by creation time, update time,
    or comment count, and ordered ascending or descending. The
    per_page parameter controls page size (1-100, GitHub defaults
    to 30).
    """
    match \exhaustive\ ut.URITemplateParse(issues_url)
    | let tpl: ut.URITemplate =>
      let u: String val =
        tpl.expand(ut.URITemplateVariables)
      let params =
        recover val
      ...
      GetRepositoryIssues.by_url(
        u + req.QueryParams(params), _creds)
    | let e: ut.URITemplateParseError =>
      Promise[(PaginatedList[Issue] | req.RequestError)]
        .> apply(
          req.RequestError(
  ...
  fun get_pull_request(number: I64)
    : Promise[PullRequestOrError]
  =>
    """
    Fetches a single pull request from this repository by its
    number.
    """
...
primitive GetRepository
  ...
  =>
    """
    Fetches a repository by direct API URL.
    """
    let p = Promise[RepositoryOrError]
    let r =
      req.ResultReceiver[Repository](
        creds,
        p,
...
primitive GetRepositoryLabels
  ...
  fun apply(
    ...
    repo: String,
    creds: req.Credentials)
    : Promise[(PaginatedList[Label] | req.RequestError)]
  =>
    """
    Fetches labels by owner and repo name.
    """
    match \exhaustive\ ut.URITemplateParse(
  ...
  =>
    """
    Fetches labels by direct API URL.
    """
    let lc = LabelJSONConverter
    let plc =
      PaginatedListJSONConverter[Label](creds, lc)
    let p =
      Promise[(PaginatedList[Label] | req.RequestError)]
    let r =
      PaginatedResultReceiver[Label](creds, p, plc)
...
primitive GetOrganizationRepositories
  """
  Lists all repositories in a GitHub organization as a paginated
  list.
  """
  fun apply(
    org: String,
  ...
  =>
    """
    Lists repositories for the given organization.
    """
    match \exhaustive\ ut.URITemplateParse(
    ...
    | let e: ut.URITemplateParseError =>
      Promise[
        (PaginatedList[Repository] | req.RequestError)]
        .> apply(
          req.RequestError(
            where message' = e.message))
    end
  ...
  =>
    """
    Lists repositories by direct API URL.
    """
    let rc = RepositoryJSONConverter
    let plc =
      PaginatedListJSONConverter[Repository](creds, rc)
    let p =
      Promise[
        (PaginatedList[Repository] |
          req.RequestError)]
    let r =
      PaginatedResultReceiver[Repository](creds, p, plc)

    LinkedJSONRequester(creds, url, r)
    p

primitive RepositoryJSONConverter
  is req.JSONConverter[Repository]
  """
  Converts a JSON object from the repositories API into a
  Repository.
  """
  fun apply(
    json: JSONNav,
  ...
  =>
    ...
    let owner =
      UserJSONConverter(json("owner"), creds)?
    ...
    let homepage =
      JSONNavUtil.string_or_none(json("homepage"))?
    let default_branch =
      json("default_branch").as_string()?
    let organization =
      try
    ...
    let network_count =
      try json("network_count").as_i64()?
      else None
      end
    let open_issues = json("open_issues").as_i64()?
    let open_issues_count =
      json("open_issues_count").as_i64()?
    let stargazers_count =
      json("stargazers_count").as_i64()?
    let subscribers_count =
      try json("subscribers_count").as_i64()?
      else None
      end
    let watchers = json("watchers").as_i64()?
    let watchers_count =
      json("watchers_count").as_i64()?
    let language =
      JSONNavUtil.string_or_none(json("language"))?
    let license =
      try
    ...
    let archive_url =
      json("archive_url").as_string()?
    let assignees_url =
      json("assignees_url").as_string()?
    let blobs_url = json("blobs_url").as_string()?
    let branches_url =
      json("branches_url").as_string()?
    let comments_url =
      json("comments_url").as_string()?
    let commits_url =
      json("commits_url").as_string()?
    let compare_url =
      json("compare_url").as_string()?
    let contents_url =
      json("contents_url").as_string()?
    let contributors_url =
      json("contributors_url").as_string()?
    let deployments_url =
      json("deployments_url").as_string()?
    let downloads_url =
      json("downloads_url").as_string()?
    let events_url = json("events_url").as_string()?
    let forks_url = json("forks_url").as_string()?
    let git_commits_url =
      json("git_commits_url").as_string()?
    let git_refs_url =
      json("git_refs_url").as_string()?
    let git_tags_url =
      json("git_tags_url").as_string()?
    let issue_comment_url =
      json("issue_comment_url").as_string()?
    let issue_events_url =
      json("issue_events_url").as_string()?
    let issues_url = json("issues_url").as_string()?
    let keys_url = json("keys_url").as_string()?
    let labels_url = json("labels_url").as_string()?
    let languages_url =
      json("languages_url").as_string()?
    let merges_url = json("merges_url").as_string()?
    let milestones_url =
      json("milestones_url").as_string()?
    let notifications_url =
      json("notifications_url").as_string()?
    let pulls_url = json("pulls_url").as_string()?
    let releases_url =
      json("releases_url").as_string()?
    let stargazers_url =
      json("stargazers_url").as_string()?
    let statuses_url =
      json("statuses_url").as_string()?
    let subscribers_url =
      json("subscribers_url").as_string()?
    let subscription_url =
      json("subscription_url").as_string()?
    ...
    let mirror_url =
      JSONNavUtil.string_or_none(json("mirror_url"))?
